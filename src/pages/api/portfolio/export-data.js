...
const fetchImageBuffer = async (imageUrl) => {
  try {
    // Extract file extension from URL to determine image type
    const fileExtension = imageUrl.split(".").pop().toLowerCase()
    const validExtensions = ["jpg", "jpeg", "png", "gif", "webp", "tiff", "bmp", "svg"]
    ...
    console.log(`Successfully fetched image of type: ${fileExtension}`)
...
const createHeader = async (doc, user) => {
  ...
  if (user.profile?.picture) {
    try {
      ...
      if (imageBuffer) {
        ...
        // PDFKit will automatically detect the image format
        ...
        doc.circle(centerX, centerY, radius).strokeColor(colors.primary).lineWidth(1.5).stroke()
      }
    } catch (error) {
  ...
  doc.font(fonts.bold).fontSize(18).fillColor(colors.primary).text(user.name.toUpperCase(), nameStartX, MARGIN)

  let currentY = doc.y + 8

  if (user.profile?.tagline) {
    doc.font(fonts.italic).fontSize(12).fillColor(colors.secondary).text(user.profile.tagline, nameStartX, currentY)
    currentY = doc.y + 8
  }
...
const addSection = (doc, title) => {
  ...
  doc.font(fonts.bold).fontSize(12).fillColor(colors.primary).text(title.toUpperCase(), LEFT_COLUMN)
...
const addExperienceEntry = (doc, experience) => {
  ...
  doc.font(fonts.bold).fontSize(11).fillColor(colors.text).text(experience.position, LEFT_COLUMN)
  ...
  if (experience?.url) {
    doc.fillColor(colors.accent).text(experience.company, LEFT_COLUMN, companyY, {
      link: experience.url,
      underline: true,
    })
  } else {
    doc.fillColor(colors.secondary).text(experience.company, LEFT_COLUMN, companyY)
  }
  ...
  if (experience.description) {
    ...
    const sentences = experience.description.split(/\.(?:\s+|\s*$)/).filter((s) => s.trim().length > 0)
    ...
    if (sentences.length > 1) {
      sentences.forEach((sentence, idx) => {
        ...
        doc.fillColor(colors.text).text(sentence.trim() + ".", LEFT_COLUMN + 10, doc.y, {
      ...
      doc.text(experience.description, {
        width: CONTENT_WIDTH,
        align: "left",
        lineGap: 4, // Add 1.5 line spacing
      })
    }
  }
...
const addEducationEntry = (doc, education) => {
  ...
  const degreeY = doc.y
  doc.font(fonts.bold).fontSize(11).fillColor(colors.text).text(education.degree, LEFT_COLUMN)
  ...
  if (education.url) {
    doc.fillColor(colors.accent).text(education.institution, LEFT_COLUMN, institutionY, {
      link: education.url,
      underline: true,
    })
  } else {
    doc.fillColor(colors.secondary).text(education.institution, LEFT_COLUMN, institutionY)
  }
...
const addSkills = (doc, skills) => {
  ...
  Object.entries(skillsByCategory).forEach(([category, categorySkills], index) => {
    ...
    if (Object.keys(skillsByCategory).length > 1) {
      doc.font(fonts.bold).fontSize(10).fillColor(colors.secondary).text(category, LEFT_COLUMN)
      doc.moveDown(0.3)
    }
      ...
      for (let col = 0; col < columns; col++) {
        ...
        if (skillIndex < skillNames.length) {
          ...
          if (col === 0) {
            ...
            doc.fillColor(colors.text).text(skillNames[skillIndex], x + 10, doc.y, {
            ...
            doc.fillColor(colors.text).text(skillNames[skillIndex], x + 10, rowY, {
              width: columnWidth - 20,
            })
          }
        }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const doc = new PDFDocument({
      ...
      margin: MARGIN,
      bufferPages: true,
      lineGap: 4, // Add this line to set default line spacing
    })
    ...
    if (user.profile?.bio) {
      ...
      const bioY = doc.y
      doc.font(fonts.regular).fontSize(10).fillColor(colors.text).text(user.profile.bio, LEFT_COLUMN, bioY, {
        width: CONTENT_WIDTH,
        align: "justify",
        lineGap: 4, // Add 1.5 line spacing
      })
